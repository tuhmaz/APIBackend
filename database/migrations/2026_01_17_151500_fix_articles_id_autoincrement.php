        ...
        foreach ($connections as $connection) {
            ...
            try {
                DB::connection($connection)->statement(
                    'ALTER TABLE articles MODIFY COLUMN id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT'
                );
            } catch (\Throwable $e) {
                if (str_contains($e->getMessage(), '1075')) {
                    DB::connection($connection)->statement('ALTER TABLE articles ADD PRIMARY KEY (id)');
                    DB::connection($connection)->statement(
                        'ALTER TABLE articles MODIFY COLUMN id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT'
                    );
                } else {
